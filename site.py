...
if st.session_state.etapa >= 2:
    ...
    if st.button('Calcular Propriedades'):
        ...
        st.session_state.mix = mi_eixox(h, tw, bfi, tfi, bfs, tfs, st.session_state.yc)
    ...
    if "area" in st.session_state:
        st.subheader('Resultados')
        st.write(f'Área da Seção Transversal = {st.session_state.area:.3f} mm²')
        st.write(f'Coordenada y do centróide = {st.session_state.yc:.3f} mm')
        st.write(f'Ix = {st.session_state.mix:.3f} mm⁴')
...
# Tensões normais
def sigma_x(N, M, Ix, y, yc, area):
    normal = (N * 1e3) / area
    fletor = (M * 1e6 * (yc - y)) / Ix
...
def tau_xy(Q, Qest, Ix, tw):
    cortante = (Q * 1e3 * Qest)/(Ix * tw)
    return cortante
...
if st.session_state.etapa >= 3:
    ...
    if "saida" in st.session_state:
        ...
        if st.button("Calcular Tensões"):
            ...
            st.session_state.sigma = sigma_x(Nx, Mx, Ix, y, yc, area) 
            st.session_state.tau = tau_xy(Qx, Qest, Ix, tw)
